...
def generate_user(user_id):
    ...
    user = {
        ...
        "profile": {
            ...
            "interests": random.sample(["coding", "reading", "hiking", "gaming", "music", "travel", "sports", "photography"], k=random.randint(2, 5)),
            "skills": random.sample(["JavaScript", "Python", "Rust", "Go", "Java", "C++", "React", "Node.js", "SQL", "AWS"], k=random.randint(2, 6)),
            "education": [
                {
                    "institution": random.choice(["MIT", "Stanford", "Harvard", "UC Berkeley", "Carnegie Mellon", "State University"]),
                    "degree": random.choice(["BS Computer Science", "MS Engineering", "MBA", "PhD", "BA Business"]),
            ...
            "experience": [
                {
                    ...
                    "position": random.choice(["Software Engineer", "Senior Developer", "Tech Lead", "Manager", "Director"]),
        ...
        "socialMedia": {
            "connections": [f"user{random.randint(1, 10000)}" for _ in range(random.randint(0, 20))],
            "posts": [
                {
                    "key": f"post{i}",
...
def main():
    ...
    # Ask user how many records to generate
    try:
        num_users = int(input("How many users to generate? (default: 1000): ") or "1000")
    except ValueError:
        num_users = 1000
    ...
    for batch_start in range(1, num_users + 1, batch_size):
        ...
        if batch:
            ...
            print(f"Inserted batch {batch_start}-{batch_end-1} ({total_inserted}/{num_users})")
    
    # Verify
    final_count = collection.count_documents({})
    print(f"\n✓ Successfully generated {final_count} users in MongoDB!")
